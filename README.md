...
END;
```
<!-- TOC --><a name="object-types"></a>
### Object Types
``` sql
CREATE OR REPLACE TYPE Person_test_schema AS OBJECT (
    first_name VARCHAR2(50),
    last_name VARCHAR2(50),
    age NUMBER,
    
    -- Method to display full name
    MEMBER FUNCTION full_name RETURN VARCHAR2 --(OPTIONAL)
);
/
```

<!-- TOC --><a name="table-type"></a>
### Table-Type

``` 1) Creating table type using objects```
``` sql
-- Create a nested table type that holds multiple Person objects
CREATE OR REPLACE TYPE PersonTable AS TABLE OF Person;
/

```
``` 2) Creating a table type using %rowtype```
<h4 style="color:#00FF00; algin:center; text-align:justify"><b>(Recommened)</b>:</h4>
<span style=" algin:center; text-align:justify">It ensures that your record or collection structure is always synchronized with the actual table schema, making it easier to handle changes in the table’s structure (e.g., adding or removing columns).
</span>

```sql
-- Create a nested table type to store rows of the 'employees' table
